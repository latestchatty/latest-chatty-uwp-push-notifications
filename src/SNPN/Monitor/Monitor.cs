...
class Monitor : IDisposable
{
	...
	public Monitor(ILogger<Monitor> logger, Func<NewEventHandler> createHandlerFunc, INetworkService networkService)
	{
		_logger = logger;
	...
	public void Start()
	{
		...
		_logger.LogTrace("Notification monitor started.");
	}
	...
	public void Stop()
	{
		...
		_logger.LogTrace("Notification monitor stopped.");
	}

	private async void TimerCallback(object state)
	{
		var parser = new EventParser();
		_logger.LogTrace("Waiting for next monitor event...");
		try
		{
			if (_lastEventId == 0)
			{
		...
		catch (Exception ex)
		{
			...
			if (ex is TaskCanceledException)
			{
				...
				_logger.LogTrace("Timed out waiting for winchatty.");
			}
			else
			{
				...
				_lastEventId = 0;
				_logger.LogError(ex, $"Exception in {nameof(TimerCallback)}");
			}
		}
		finally
		{
			if (_timerEnabled)
			{
				_logger.LogTrace("Delaying next monitor for {monitorDelay}ms", _timeDelay * 1000);
